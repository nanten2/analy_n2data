...
for i in range(5):
    xscan_Ta[i] = baseline_fitting(arr_x, xscan_Ta[i], 29000,29500, 30000, 30500)#12CO1-0(29,30,31,32)
    #xscan_Ta[i] = baseline_fitting(arr_x, xscan_Ta[i], 11000,11228, 12800, 13000)#12CO1-0(29,30,31,32)    
    yscan_Ta[i] = baseline_fitting(arr_x, yscan_Ta[i], 29000,29500,30000, 30500)#29,818
...
#try:    
    ...
    for i in [2,7,11,12,13,14,17,10,22]:
        axlist[i].set_ylim(-2, 20
                          )
        axlist[i].set_xlim(1750, 1850)
    ...
    if savefig:
        plt.savefig("/home/amigos/result/pointing/{}_13CO.png".format(os.path.dirname(dirname)))
        pass
    if plot:
        plt.show()
    else:
...
f_ = open('./hosei_230_20191211.txt', 'r')
