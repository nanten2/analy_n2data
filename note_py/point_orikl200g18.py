...
for i in range(5):
    xscan_Ta[i] = baseline_fitting(arr_x, xscan_Ta[i], 15000,15500, 16300, 16800)#12CO1-0(29,30,31,32)
    #xscan_Ta[i] = baseline_fitting(arr_x, xscan_Ta[i], 11000,11228, 12800, 13000)#12CO1-0(29,30,31,32)    
    yscan_Ta[i] = baseline_fitting(arr_x, yscan_Ta[i], 15000,15500,16300, 16800)
...
#try:    
    ...
    for i in [2,7,11,12,13,14,17,10,22]:
        axlist[i].set_ylim(-2, 5
                          )
        axlist[i].set_xlim(950, 1000)
    ...
    plt.text(0, -0.5, "DATA PATH :   {}".format(dirname), fontsize=6)

    [a.grid() for a in axlist]

finally:
    if savefig:
        plt.savefig("/home/amigos/result/pointing/{}_C18O.png".format(os.path.dirname(dirname)))
        pass
    if plot:
        plt.show()
    else:
